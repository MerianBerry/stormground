...
#    define PATH_MAX 256
#  endif
/*typedef HANDLE (WINAPI *cwtexw) (LPSECURITY_ATTRIBUTES, LPCWSTR, DWORD,
DWORD); static HMODULE k32                    = NULL; static cwtexw
CreateWaitableTimerExW = NULL;
...
char *str_add (char *lhs, char const *rhs) {
  ...
  size_t rhss = strlen (rhs);
  size_t size = lhss + strlen (rhs);
  char  *buf  = (char *)malloc (size + 1);
  buf[size]   = 0;
  if (lhs) {
    memcpy (buf, lhs, lhss);
  }
  memcpy (buf + lhss, rhs, rhss);
  return buf;
}
...
size_t str_fli (char const *str, char const *cmp) {
  ...
  long         i;
  size_t const s1 = strlen (str);
  size_t const s2 = strlen (cmp);
  if (s2 > s1) {
    return npos;
  }
  for (i = s1 - (s2); i >= 0; --i) {
...
char *str_append (char *src, char const *nstr, size_t bytes) {
  ...
  size_t const l   = src ? strlen (src) : 0;
  size_t const l2  = MIN (strlen (nstr), bytes);
